...
export default function ImobiliarioPage() {
  ...
  const [opts, setOpts] = useState<{ anos: number[]; especies: string[] }>({ anos: [], especies: [] })
  ...
  useEffect(() => {
    fetch('/api/orcamento/filtros')
      ...
      .then(d => {
        if (d && !d.error) {
          setOpts({ anos: d.anos ?? [], especies: d.especies ?? [] })
  ...
  const filtros: FiltrosImobiliario = { ano: rAno, mes: rMes, especie: rEsp }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#eef2f9', padding: '26px 14px', fontFamily: "var(--font-poppins), 'Poppins', sans-serif" }}>
      <div style={{ maxWidth: 1560, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '18px 0 0', flexWrap: 'wrap', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            ...
            <select aria-label="Espécie" value={rEsp} onChange={e => setREsp(e.target.value)} style={selectPill}>
              ...
              {opts.especies.map(esp => <option key={esp} value={esp}>{esp}</option>)}
            </select>
          </div>
        ...
        {/* ===== PAINEL ===== */}
        <PainelImobiliario filtros={filtros} />
