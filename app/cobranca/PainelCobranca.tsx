  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {/* Resultado Mensal da Arrecadação — DAM Geradas × DAM Pagas pelo setor de Cobrança,
          ...
          compDamId.porMes — mesma fonte que alimentava o card "Comparativo de DAM por ID"
          (removido a pedido do usuário depois que essa barra passou pra cá). Conta DOCUMENTOS
          distintos (COUNT DISTINCT cd_guia) em vez de eventos de baixa, por isso é sempre ≤
          "Pagas" (uma guia parcelada gera vários eventos de baixa, um por parcela). */}
            ...
            <>
              ...
              {resultadoDrillMes ? (
                ...
                <>
                  ...
                  <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                    {rm.porMes.filter(m => m.geradas > 0 || m.pagas > 0).map(m => (
                      <button key={m.mes} onClick={() => selecionarResultadoMes(rm.ano, m.mes)}
                        title={`Detalhar ${MESES_ABREV[m.mes - 1]}/${rm.ano} por tributo`}
      ...
      {/* Mapa de calor (treemap) — Top 10 Mês/Ano por DAM Geradas, a pedido do usuário
          ...
          Tributo passou por aqui como 3ª coluna e depois foi pra junto de "Baixas Processadas
          por Ano"; agora quem entra como 3ª coluna é Canais de Arrecadação, que saiu de
          largura total (reordenação a pedido do usuário). */}
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 0.8fr 0.8fr', gap: 18, marginTop: 18, alignItems: 'stretch' }}>
        <div style={card}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
            <div>
