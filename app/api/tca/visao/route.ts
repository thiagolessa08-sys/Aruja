...
import { bucketsTca, qtdImoveisTca, dataAtualizacaoTca, type BucketsTcaAno } from '@/lib/tca-engine'
...
export async function GET(req: NextRequest) {
  ...
  try {
    const [buckets, qtdImoveis, dataAtualizacao] = await Promise.all([
      bucketsTca(),
      qtdImoveisTca(),
      dataAtualizacaoTca(),
    ])
    ...
    const zero: BucketsTcaAno = { lancado: 0, arrecadado: 0, emAberto: 0, inadimplente: 0, isento: 0, suspenso: 0 }
    ...
    const cards = {
      ...
      imoveis: cmp(qtdImoveis.get(anoRef) ?? 0, qtdImoveis.get(anoAnt) ?? 0),
    }
