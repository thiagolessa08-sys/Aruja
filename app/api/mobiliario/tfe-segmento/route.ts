...
// `ano`/`mes` (opcionais) seguem a mesma convenção do PainelTributo: ano restringe ao
// exercício de lançamento, mes acumula as parcelas com vencimento até aquele mês.
async function tfePorSegmento(ano?: number, mes?: number): Promise<{ nome: string; valor: number }[]> {
  return cached(`tfe:porSegmento:${ano ?? ''}:${mes ?? ''}`, TTL_15MIN, async () => {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ porSegmento: await tfePorSegmento(ano, mes) })
  } catch (e) {
