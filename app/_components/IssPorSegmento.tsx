...
// "ISS por Segmento" — ISS/ISSQN (grupo iss, cd_tributo em lib/tributos.ts) lançado,
// agrupado pelo segmento (ds_grupo) da empresa vinculada à guia. Fonte:
// /api/mobiliario/iss-segmento. `ano`/`mes` (opcionais) refletem os filtros do painel
// (Exercício/Mês), mesma convenção usada pelo PainelTributo/TfePorSegmento.
export default function IssPorSegmento({ ano, mes }: { ano?: number; mes?: number }) {
  ...
  return (
    <div style={{ ...card, marginTop: 18 }}>
      ...
      <div style={{ fontSize: 11, color: '#9098a8', marginTop: 2 }}>
        ISS/ISSQN lançado por segmento da empresa (ds_grupo do cadastro mobiliário){ano ? ` — Exercício ${ano}` : ''}{mes ? ` até o mês ${mes}` : ''}. &quot;Não classificado&quot; inclui guias sem vínculo com o cadastro mobiliário (ex.: retenções e exercícios mais antigos).
      </div>
