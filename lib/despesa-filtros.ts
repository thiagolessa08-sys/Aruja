...
export function whereUO(skCol = 'f.SK_INSTITUCIONAL', secretaria?: string | null): string {
  let w = ` AND ${skCol} IN (SELECT i.SK_INSTITUCIONAL FROM ${SCHEMA}.DIM_BIORC_INSTITUCIONAL i WHERE i.CD_UO >= '02.01.00' AND i.CD_UO <= '02.19.99'`
  // Secretaria selecionada = todas as sub-unidades cujo CD_UO começa com o prefixo (ex '02.04.%')
  if (secretaria) w += ` AND i.CD_UO LIKE '${secretaria}.%'`
  return w + ')'
}
